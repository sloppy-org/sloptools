...
func (s *Store) CompleteItemBySource(source, sourceRef string) error {
	...
	res, err := s.db.Exec(`UPDATE items
		 ...
		 WHERE source = ? AND source_ref = ? AND state != ?`, ItemStateDone, cleanSource, cleanSourceRef, ItemStateDone)
	if err != nil {
		return err
	}
	...
	if err := s.db.QueryRow(`SELECT COUNT(*) FROM items WHERE source = ? AND source_ref = ?`, cleanSource, cleanSourceRef).Scan(&existingCount); err != nil {
		return err
	}
	if existingCount == 0 {
...
func (s *Store) UpdateItemTimes(id int64, visibleAfter, followUpAt *string) error {
	res, err := s.db.Exec(`UPDATE items
		 ...
		 WHERE id = ?`, normalizeOptionalString(visibleAfter), normalizeOptionalString(followUpAt), id)
	if err != nil {
		return err
	}
	affected, err := res.RowsAffected()
...
func (s *Store) ResurfaceDueItems(now time.Time) (int, error) {
	...
	res, err := s.db.Exec(`UPDATE items
		 ...
		 WHERE state = ?
		   ...
		   )`, ItemStateInbox, ItemStateWaiting, cutoff, cutoff)
	if err != nil {
		return 0, err
	}
	affected, err := res.RowsAffected()
...
func (s *Store) CountItemsByStateFiltered(now time.Time, filter ItemListFilter) (map[string]int, error) {
	counts := map[string]int{ItemStateInbox: 0, ItemStateWaiting: 0, ItemStateSomeday: 0, ItemStateDone: 0}
	normalizedFilter, err := s.prepareItemListFilter(filter)
	if err != nil {
		return nil, err
	}
...
FROM items
`
	args := []any{ItemStateInbox, cutoff, ItemStateWaiting, ItemStateSomeday, ItemStateDone}
	parts := []string{}
	parts, args = appendItemFilterClauses(parts, args, normalizedFilter, "")
	if len(parts) > 0 {
