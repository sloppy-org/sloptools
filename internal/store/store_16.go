...
func scopeContextIDTx(tx *sql.Tx, scope string) (int64, error) {
	...
	if err := tx.QueryRow(`SELECT id
		 ...
		 WHERE parent_id IS NULL
		   AND lower(name) = lower(?)`, cleanScope).Scan(&contextID); err != nil {
		return 0, err
	}
...
func (s *Store) syncScopedContextLinkTx(tx *sql.Tx, linkTable, entityColumn string, entityID int64, scope string) error {
	...
	if _, err := tx.Exec(`DELETE FROM `+linkTable+`
		 WHERE `+entityColumn+` = ?
		   AND context_id IN (
		     SELECT id
		     FROM contexts
	...
	_, err = tx.Exec(`INSERT OR IGNORE INTO `+linkTable+` (context_id, `+entityColumn+`)
		 VALUES (?, ?)`, contextID, entityID)
	return err
}
...
func (s *Store) syncScopedContextLink(linkTable, entityColumn string, entityID int64, scope string) error {
	...
	if _, err := s.db.Exec(`DELETE FROM `+linkTable+`
		 WHERE `+entityColumn+` = ?
		   AND context_id IN (
		     SELECT id
		     FROM contexts
	...
	_, err = s.db.Exec(`INSERT OR IGNORE INTO `+linkTable+` (context_id, `+entityColumn+`)
		 VALUES (?, ?)`, contextID, entityID)
	return err
}
...
func (s *Store) migrateSphereToContextSupport() (err error) {
	...
	if !tableColumns["workspaces"]["sphere"] && !tableColumns["items"]["sphere"] && !tableColumns["time_entries"]["sphere"] && !tableColumns["external_accounts"]["sphere"] && !tableColumns["external_container_mappings"]["sphere"] {
		return nil
	}
...
func scanTimeEntry(scanner interface{ Scan(dest ...any) error }) (TimeEntry, error) {
	var (
		out       TimeEntry
	...
	if err := scanner.Scan(&out.ID, &workspace, &out.Sphere, &out.StartedAt, &endedAt, &out.Activity, &notes); err != nil {
