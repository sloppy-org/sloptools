...
func (s *Store) UpsertWorkspaceWatch(workspaceID int64, configJSON string, pollIntervalSeconds int, enabled bool, currentBatchID *int64) (WorkspaceWatch, error) {
	...
	_, err = s.db.Exec(`INSERT INTO workspace_watches (workspace_id, config_json, poll_interval_seconds, enabled, current_batch_id)
		 VALUES (?, ?, ?, ?, ?)
		 ON CONFLICT(workspace_id) DO UPDATE SET
		   ...
		   updated_at = datetime('now')`, workspaceID, cleanConfig, cleanInterval, boolToSQLiteInt(enabled), normalizeOptionalInt64(currentBatchID))
	if err != nil {
		return WorkspaceWatch{}, err
	}
...
func (s *Store) SetWorkspaceWatchEnabled(workspaceID int64, enabled bool) (WorkspaceWatch, error) {
	res, err := s.db.Exec(`UPDATE workspace_watches
		 SET enabled = ?, updated_at = datetime('now')
		 WHERE workspace_id = ?`, boolToSQLiteInt(enabled), workspaceID)
	if err != nil {
		return WorkspaceWatch{}, err
	}
...
func (s *Store) SetWorkspaceWatchBatch(workspaceID int64, batchID *int64) (WorkspaceWatch, error) {
	...
	res, err := s.db.Exec(`UPDATE workspace_watches
		 ...
		 WHERE workspace_id = ?`, normalizeOptionalInt64(batchID), workspaceID)
	if err != nil {
		return WorkspaceWatch{}, err
	}
	affected, err := res.RowsAffected()
...
func (s *Store) TableColumnNames(table string) ([]string, error) {
	return s.tableColumnNames(table)
}

func WorkspaceIDString(id int64) string {
	return workspaceIDString(id)
}

func ParseWorkspaceTimestamp(value string) int64 {
	return parseWorkspaceTimestamp(value)
}

func InvalidWorkspaceIDError(id string) error {
	return invalidWorkspaceIDError(id)
}

func (s *Store) ActiveEnrichedWorkspace() (Workspace, error) {
	return s.activeEnrichedWorkspace()
}

func (s *Store) AppServerModelProfileForWorkspacePath(path string) string {
	return s.appServerModelProfileForWorkspacePath(path)
}

func NormalizeWorkspaceCompatName(name string) string {
	return normalizeWorkspaceCompatName(name)
}

func NormalizeWorkspaceCompatChatModel(raw string) string {
	return normalizeWorkspaceCompatChatModel(raw)
}

func NormalizeWorkspaceCompatChatModelReasoningEffort(raw string) string {
	return normalizeWorkspaceCompatChatModelReasoningEffort(raw)
}
...
func NormalizeRenderFormat(format string) string {
	return normalizeRenderFormat(format)
}
