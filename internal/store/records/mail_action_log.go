...
func CreateMailActionLog(db interface {
	Exec(string, ...any) (sql.Result, error)
}, get func(int64) (MailActionLog, error), normalizeProvider func(string) string, input MailActionLogInput) (MailActionLog, error) {
	...
	result, err := db.Exec(`INSERT INTO mail_action_logs (
...
func GetMailActionLog(db interface{ QueryRow(string, ...any) *sql.Row }, id int64) (MailActionLog, error) {
	if id <= 0 {
		return MailActionLog{}, errors.New("id is required")
	}
	var log MailActionLog
	err := db.QueryRow(`SELECT id, account_id, provider, message_id, resolved_message_id, action, folder_from, folder_to, subject, sender, request_json, status, error_text, created_at, updated_at
...
func UpdateMailActionLogResult(db interface {
	Exec(string, ...any) (sql.Result, error)
}, id int64, status, resolvedMessageID, errorText string) error {
	if id <= 0 {
	...
	_, err := db.Exec(`UPDATE mail_action_logs
...
func ListMailActionLogs(db interface {
	Query(string, ...any) (*sql.Rows, error)
}, accountID int64, limit int) ([]MailActionLog, error) {
	if accountID <= 0 {
	...
	rows, err := db.Query(`SELECT id, account_id, provider, message_id, resolved_message_id, action, folder_from, folder_to, subject, sender, request_json, status, error_text, created_at, updated_at
FROM mail_action_logs
WHERE account_id = ?
