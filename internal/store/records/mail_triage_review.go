...
func CreateMailTriageReview(db interface {
	Exec(string, ...any) (sql.Result, error)
}, get func(int64) (MailTriageReview, error), normalizeProvider func(string) string, input MailTriageReviewInput) (MailTriageReview, error) {
	...
	result, err := db.Exec(`INSERT INTO mail_triage_reviews (
...
func GetMailTriageReview(db interface{ QueryRow(string, ...any) *sql.Row }, id int64) (MailTriageReview, error) {
	if id <= 0 {
		return MailTriageReview{}, errors.New("id is required")
	}
	var review MailTriageReview
	err := db.QueryRow(`SELECT id, account_id, provider, message_id, folder, subject, sender, action, created_at
...
func ListMailTriageReviews(db interface {
	Query(string, ...any) (*sql.Rows, error)
}, accountID int64, limit int) ([]MailTriageReview, error) {
	if accountID <= 0 {
	...
	rows, err := db.Query(`SELECT id, account_id, provider, message_id, folder, subject, sender, action, created_at
FROM mail_triage_reviews
WHERE account_id = ?
...
func ListMailTriageReviewedMessageIDs(db interface {
	Query(string, ...any) (*sql.Rows, error)
}, accountID int64, folder string, limit int) ([]string, error) {
	if accountID <= 0 {
		return nil, errors.New("account_id is required")
	}
	if limit <= 0 {
	...
	rows, err := db.Query(`SELECT message_id
FROM mail_triage_reviews
WHERE account_id = ?
...
func DeleteMailTriageReview(db interface {
	Exec(string, ...any) (sql.Result, error)
}, id int64) error {
	if id <= 0 {
		return errors.New("id is required")
	}
	_, err := db.Exec(`DELETE FROM mail_triage_reviews WHERE id = ?`, id)
	return err
}
