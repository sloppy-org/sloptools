...
func GetPushRegistration(db *sql.DB, id int64) (PushRegistration, error) {
	if id <= 0 {
		return PushRegistration{}, errors.New("push registration id is required")
	}
	return scanPushRegistration(db.QueryRow(
...
func UpsertPushRegistration(db *sql.DB, reg PushRegistration) (PushRegistration, error) {
	...
	return GetPushRegistration(db, existingID)
}

func ListPushRegistrations(db *sql.DB) ([]PushRegistration, error) {
	rows, err := db.Query(
		`SELECT id, session_id, workspace_id, platform, device_token, device_label, created_at, updated_at
		   FROM push_registrations
		  ORDER BY id ASC`,
...
func ListPushRegistrationsForChatSession(db *sql.DB, sessionID string, workspaceID int64) ([]PushRegistration, error) {
