...
func (s *Store) migrateProjectRemovalSupport() error {
	...
	if !tableColumns["workspaces"]["project_id"] && !tableColumns["items"]["project_id"] && !tableColumns["time_entries"]["project_id"] && !tableColumns["external_container_mappings"]["project_id"] {
...
func (s *Store) repairProjectLegacyForeignKeyTargets() error {
	specs := []projectLegacyForeignKeyRepairSpec{{table: "workspace_artifact_links", createTableSQL: `CREATE TABLE workspace_artifact_links (
  workspace_id INTEGER NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  artifact_id INTEGER NOT NULL REFERENCES artifacts(id) ON DELETE CASCADE,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  PRIMARY KEY (workspace_id, artifact_id)
)`, columns: []string{"workspace_id", "artifact_id", "created_at"}}, {table: "external_bindings", createTableSQL: `CREATE TABLE external_bindings (
  id INTEGER PRIMARY KEY,
...
);`, postCopySQL: []string{`CREATE UNIQUE INDEX idx_external_bindings_identity
  ON external_bindings(account_id, provider, object_type, remote_id)`, `CREATE INDEX idx_external_bindings_stale
  ON external_bindings(provider, last_synced_at)`}, columns: []string{"id", "account_id", "provider", "object_type", "remote_id", "item_id", "artifact_id", "container_ref", "remote_updated_at", "last_synced_at"}}, {table: "item_artifacts", createTableSQL: `CREATE TABLE item_artifacts (
  item_id INTEGER NOT NULL REFERENCES items(id) ON DELETE CASCADE,
  artifact_id INTEGER NOT NULL REFERENCES artifacts(id) ON DELETE CASCADE,
  role TEXT NOT NULL DEFAULT 'related' CHECK (role IN ('source', 'related', 'output')),
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  PRIMARY KEY (item_id, artifact_id)
);`, postCopySQL: []string{`CREATE INDEX idx_item_artifacts_artifact_id
  ON item_artifacts(artifact_id)`}, columns: []string{"item_id", "artifact_id", "role", "created_at"}}, {table: "batch_runs", createTableSQL: `CREATE TABLE batch_runs (
  id INTEGER PRIMARY KEY,
  workspace_id INTEGER NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
...
);`, postCopySQL: []string{`CREATE INDEX idx_batch_runs_workspace_started
  ON batch_runs(workspace_id, datetime(started_at) DESC, id DESC)`}, columns: []string{"id", "workspace_id", "started_at", "finished_at", "config_json", "status"}}, {table: "batch_run_items", createTableSQL: `CREATE TABLE batch_run_items (
...
);`, postCopySQL: []string{`CREATE INDEX idx_batch_run_items_batch_status
  ON batch_run_items(batch_id, status, item_id)`}, columns: []string{"batch_id", "item_id", "status", "pr_number", "pr_url", "error_msg", "started_at", "finished_at"}}, {table: "workspace_watches", createTableSQL: `CREATE TABLE workspace_watches (
  ...
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now'))
)`, columns: []string{"workspace_id", "config_json", "poll_interval_seconds", "enabled", "current_batch_id", "created_at", "updated_at"}}, {table: "context_items", createTableSQL: `CREATE TABLE context_items (
  context_id INTEGER NOT NULL REFERENCES contexts(id) ON DELETE CASCADE,
  item_id INTEGER NOT NULL REFERENCES items(id) ON DELETE CASCADE,
  PRIMARY KEY (context_id, item_id)
)`, columns: []string{"context_id", "item_id"}}, {table: "context_workspaces", createTableSQL: `CREATE TABLE context_workspaces (
  context_id INTEGER NOT NULL REFERENCES contexts(id) ON DELETE CASCADE,
  workspace_id INTEGER NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  PRIMARY KEY (context_id, workspace_id)
)`, columns: []string{"context_id", "workspace_id"}}, {table: "context_external_container_mappings", createTableSQL: `CREATE TABLE context_external_container_mappings (
  context_id INTEGER NOT NULL REFERENCES contexts(id) ON DELETE CASCADE,
  mapping_id INTEGER NOT NULL REFERENCES external_container_mappings(id) ON DELETE CASCADE,
  PRIMARY KEY (context_id, mapping_id)
)`, columns: []string{"context_id", "mapping_id"}}, {table: "context_time_entries", createTableSQL: `CREATE TABLE context_time_entries (
  context_id INTEGER NOT NULL REFERENCES contexts(id) ON DELETE CASCADE,
  time_entry_id INTEGER NOT NULL REFERENCES time_entries(id) ON DELETE CASCADE,
  PRIMARY KEY (context_id, time_entry_id)
)`, columns: []string{"context_id", "time_entry_id"}}, {table: "participant_sessions", createTableSQL: `CREATE TABLE participant_sessions (
  id TEXT PRIMARY KEY,
  workspace_id INTEGER NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  started_at INTEGER NOT NULL,
  ended_at INTEGER NOT NULL DEFAULT 0,
  config_json TEXT NOT NULL DEFAULT '{}'
)`, columns: []string{"id", "workspace_id", "started_at", "ended_at", "config_json"}}, {table: "chat_sessions", createTableSQL: `CREATE TABLE chat_sessions (
  id TEXT PRIMARY KEY,
...
func (s *Store) createWorkspaceRecord(name, dirPath, sphere string) (Workspace, error) {
	res, err := s.db.Exec(`INSERT INTO workspaces (name, dir_path)
		 VALUES (?, ?)`, name, dirPath)
	if err != nil {
...
func (s *Store) GetWorkspaceByPath(dirPath string) (Workspace, error) {
	return scanWorkspace(s.db.QueryRow(`SELECT id, name, dir_path, `+scopedContextSelect("context_workspaces", "workspace_id", "workspaces.id")+` AS sphere, is_active, is_daily, daily_date, mcp_url, canvas_session_id, chat_model, chat_model_reasoning_effort, companion_config_json, created_at, updated_at
		 FROM workspaces
		 WHERE dir_path = ?`, normalizeWorkspacePath(dirPath)))
}
...
func (s *Store) EnsureDailyWorkspace(date, dirPath string) (Workspace, error) {
	...
	res, err := s.db.Exec(`INSERT INTO workspaces (name, dir_path, is_daily, daily_date)
		 VALUES (?, ?, 1, ?)`, dailyWorkspaceName(cleanDate), cleanPath, cleanDate)
	if err != nil {
		if isUniqueConstraintError(err) {
