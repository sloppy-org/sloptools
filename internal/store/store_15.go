...
import (
	...
	"errors"
	"fmt"
	"github.com/sloppy-org/sloptools/internal/store/records"
	"strings"
	"time"
)
...
func scanParticipantSession(scanner interface{ Scan(...any) error }) (ParticipantSession, error) {
	...
	if err := scanner.Scan(&out.ID, &out.WorkspaceID, &out.WorkspacePath, &out.StartedAt, &out.EndedAt, &out.ConfigJSON); err != nil {
...
func (s *Store) AddParticipantSessionForWorkspace(workspaceID int64, configJSON string) (ParticipantSession, error) {
	...
	_, err := s.db.Exec(`INSERT INTO participant_sessions (id, workspace_id, started_at, ended_at, config_json) VALUES (?,?,?,0,?)`, id, workspaceID, now, configJSON)
	if err != nil {
		return ParticipantSession{}, err
	}
...
func (s *Store) GetParticipantSession(id string) (ParticipantSession, error) {
	session, err := scanParticipantSession(s.db.QueryRow(participantSessionSelect+` WHERE ps.id = ?`, strings.TrimSpace(id)))
	if err != nil {
		return ParticipantSession{}, err
	}
...
func (s *Store) ListParticipantSessions(ref string) ([]ParticipantSession, error) {
	cleanRef := strings.TrimSpace(ref)
	if cleanRef == "" {
		return s.listParticipantSessionsQuery(participantSessionSelect + ` ORDER BY ps.started_at DESC, ps.id DESC`)
	}
	workspace, err := s.resolveParticipantSessionWorkspace(cleanRef)
	if err != nil {
...
func (s *Store) ListParticipantSessionsForWorkspace(workspaceID int64) ([]ParticipantSession, error) {
	...
	return s.listParticipantSessionsQuery(participantSessionSelect+` WHERE ps.workspace_id = ? ORDER BY ps.started_at DESC, ps.id DESC`, workspaceID)
}
...
CREATE TABLE participant_sessions_new (
  ...
  config_json TEXT NOT NULL DEFAULT '{}'
	...
	for _, item := range legacy {
		if _, err := tx.Exec(`INSERT INTO participant_sessions_new (id, workspace_id, started_at, ended_at, config_json) VALUES (?,?,?,?,?)`, item.ID, item.WorkspaceID, item.StartedAt, item.EndedAt, strings.TrimSpace(item.ConfigJSON)); err != nil {
			return err
		}
	}
...
func (s *Store) AddParticipantSegment(seg ParticipantSegment) (ParticipantSegment, error) {
	...
	var endedAt int64
	if err := s.db.QueryRow(`SELECT ended_at FROM participant_sessions WHERE id = ?`, sessionID).Scan(&endedAt); err != nil {
	...
	res, err := s.db.Exec(`INSERT INTO participant_segments (session_id, start_ts, end_ts, speaker, text, model, latency_ms, committed_at, status) VALUES (?,?,?,?,?,?,?,?,?)`, sessionID, seg.StartTS, seg.EndTS, seg.Speaker, seg.Text, seg.Model, seg.LatencyMS, seg.CommittedAt, seg.Status)
	if err != nil {
		return ParticipantSegment{}, err
	}
...
func (s *Store) SearchParticipantSegments(sessionID, query string) ([]ParticipantSegment, error) {
	...
	rows, err := s.db.Query(`SELECT id, session_id, start_ts, end_ts, speaker, text, model, latency_ms, committed_at, status
		 FROM participant_segments WHERE session_id = ? AND text LIKE ? ORDER BY start_ts ASC, id ASC`, sid, "%"+q+"%")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) AddParticipantEvent(sessionID string, segmentID int64, eventType, payloadJSON string) error {
	...
	_, err := s.db.Exec(`INSERT INTO participant_events (session_id, segment_id, event_type, payload_json, created_at) VALUES (?,?,?,?,?)`, sid, segmentID, strings.TrimSpace(eventType), payloadJSON, time.Now().Unix())
	return err
}
...
func (s *Store) ListParticipantEvents(sessionID string) ([]ParticipantEvent, error) {
	...
	rows, err := s.db.Query(`SELECT id, session_id, segment_id, event_type, payload_json, created_at FROM participant_events WHERE session_id = ? ORDER BY created_at ASC, id ASC`, sid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) UpsertParticipantRoomState(sessionID, summaryText, entitiesJSON, topicTimelineJSON string) error {
	...
	now := time.Now().Unix()
	_, err := s.db.Exec(`INSERT INTO participant_room_state (session_id, summary_text, entities_json, topic_timeline_json, updated_at)
		 VALUES (?,?,?,?,?)
		 ON CONFLICT(session_id) DO UPDATE SET summary_text=excluded.summary_text, entities_json=excluded.entities_json, topic_timeline_json=excluded.topic_timeline_json, updated_at=excluded.updated_at`, sid, summaryText, entitiesJSON, topicTimelineJSON, now)
	return err
}
...
func (s *Store) GetParticipantRoomState(sessionID string) (ParticipantRoomState, error) {
	...
	err := s.db.QueryRow(`SELECT id, session_id, summary_text, entities_json, topic_timeline_json, updated_at FROM participant_room_state WHERE session_id = ?`, sid).Scan(&out.ID, &out.SessionID, &out.SummaryText, &out.EntitiesJSON, &out.TopicTimelineJSON, &out.UpdatedAt)
	if err != nil {
...
const (
	MailActionLogPending         = records.MailActionLogPending
	MailActionLogApplied         = records.MailActionLogApplied
	MailActionLogFailed          = records.MailActionLogFailed
	MailActionLogReconcileFailed = records.MailActionLogReconcileFailed
)

type PushRegistration = records.PushRegistration

type MailActionLog = records.MailActionLog

type MailActionLogInput = records.MailActionLogInput

type MailTriageReview = records.MailTriageReview

type MailTriageReviewInput = records.MailTriageReviewInput

func (s *Store) GetPushRegistration(id int64) (PushRegistration, error) {
	return records.GetPushRegistration(s.db, id)
}

func (s *Store) UpsertPushRegistration(reg PushRegistration) (PushRegistration, error) {
	return records.UpsertPushRegistration(s.db, reg)
}

func (s *Store) ListPushRegistrations() ([]PushRegistration, error) {
	return records.ListPushRegistrations(s.db)
}

func (s *Store) ListPushRegistrationsForChatSession(sessionID string, workspaceID int64) ([]PushRegistration, error) {
	return records.ListPushRegistrationsForChatSession(s.db, sessionID, workspaceID)
}

func (s *Store) CreateMailActionLog(input MailActionLogInput) (MailActionLog, error) {
	return records.CreateMailActionLog(s.db, s.GetMailActionLog, normalizeExternalAccountProvider, input)
}

func (s *Store) GetMailActionLog(id int64) (MailActionLog, error) {
	return records.GetMailActionLog(s.db, id)
}

func (s *Store) UpdateMailActionLogResult(id int64, status, resolvedMessageID, errorText string) error {
	return records.UpdateMailActionLogResult(s.db, id, status, resolvedMessageID, errorText)
}

func (s *Store) ListMailActionLogs(accountID int64, limit int) ([]MailActionLog, error) {
	return records.ListMailActionLogs(s.db, accountID, limit)
}

func (s *Store) CreateMailTriageReview(input MailTriageReviewInput) (MailTriageReview, error) {
	return records.CreateMailTriageReview(s.db, s.GetMailTriageReview, normalizeExternalAccountProvider, input)
}
