...
func Run(ctx context.Context, opts RunOpts) (*RunReport, error) {
	...
	for i, p := range opts.Picks {
		fmt.Fprintf(os.Stderr, "brain night: Scout starts checking %s (%d of %d).\n", p.Path, i+1, len(opts.Picks))
		entry := runOnePick(ctx, opts, dir, stagePrompt, p, broken)
		if entry.Skipped {
			fmt.Fprintf(os.Stderr, "brain night: Scout skipped %s: %s\n", p.Path, entry.Reason)
		} else {
			fmt.Fprintf(os.Stderr, "brain night: Scout finished %s. Report: %s\n", p.Path, entry.ReportPath)
		}
		if entry.Skipped {
			report.Skipped++
		}
