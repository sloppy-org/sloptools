...
func writeEscalatePrompt(runID string) (string, error) {
	...
	body := strings.Join([]string{
		...
		"with fresh, traceable evidence. If helpy web_search was capped, empty,",
		"or not useful in the bulk report, use your native web search from the",
		"prompt instead of retrying the capped helpy web_search path. Use sloppy",
		"and helpy MCP tools for local brain, Zotero, TU Graz, PDF, and fetched",
		"source checks. Do not just rewrite the bulk report: address each conflict",
		"and each open question with a fresh answer. Output Markdown with the same section structure as the bulk",
