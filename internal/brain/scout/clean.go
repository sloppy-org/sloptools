...
// sometimes glues around a structured scout report. The bulk pass and
// the self-resolve pass both showed cases where:
//
//   - the model emits chain-of-thought ("Now I have all the evidence...")
//     before the first `# Scout report` heading;
//   - the model appends a separator + bold "**Note on methodology**:..."
//     block after the last section.
//
// Tighter prompts reduce the frequency, but a deterministic trim is the
// only thing that guarantees the saved artifact is the report alone.
// We trim leading content before the first column-zero `# ` ATX h1 and,
// at the trailing end, any block that begins with a `---` horizontal
// rule whose next non-blank line is a `**...**`-style narration label
// (Note, Methodology, Disclaimer, etc.).
