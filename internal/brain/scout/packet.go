...
func buildScoutPacket(brainRoot string, p Pick) string {
	...
	b.WriteString(strings.Join([]string{
		...
		"Use helpy `web_search`, `web_fetch`, `web_fetch_packet`, `helpy_zotero` with `action=packets` and query fields under `args`, `tugonline_*`, `tu4u_*`, `pdf_read` for external lookups; helpy `pdf_read` is the only sanctioned PDF reader (modes: metadata | text with `pages` and `max_bytes` | outline; image-only PDFs return status:image_only). Per-scout-item caps: web_search ≤ 2, web_fetch ≤ 4, zotero ≤ 4, tugonline ≤ 8, tu4u ≤ 8, pdf_read ≤ 6 — a quota-exceeded message means use another source or write the report from evidence already gathered; do not retry the same capped tool. Use `helpy_tool_help tool_family=<family>` or `action=help` on any helpy_* tool to discover real action names.",
		"Use sloppy `sloppy_brain`, `sloppy_contacts`, `sloppy_calendar`, `sloppy_mail`, `sloppy_chat`, and `sloppy_source` for vault, groupware, Zulip/chat, and GitHub issue/PR/code cross-checks before public web; for GitHub pass an explicit `repo` owner/name so the lookup hits the intended project. For work-sphere chat use `sloppy_chat` with action=message_search, stream/topic/query/after/before when the entity likely appears in Zulip discussion.",
