...
func writeSleepMission(b *strings.Builder, autonomy string, sphere Sphere) {
	...
	fmt.Fprintln(b, "Episodic vs semantic split. Write today's verbatim daily log to `brain/episodic/<YYYY-MM-DD>.md` (create the file if it does not exist) summarising the activity table above as one bullet per important event with `[[people/Name]]`, `[[projects/X]]`, `[[institutions/Y]]`, `[[topics/Z]]` wikilinks. Items in the activity tables with `Imp ≥ 7` belong both in the episodic log AND as in-place edits to the canonical entity notes. Items scored 4-6 belong only in the episodic log. Items scored <4 are noise unless tied to other strong signals; do not invent canonical entries for them.")
...
func writeSleepReturnContract(b *strings.Builder, autonomy string) {
	...
	fmt.Fprintln(b, "- canonical-note content edits made (path + one-line summary per note; this is the half the prior run skipped)")
	fmt.Fprintln(b, "- new canonical notes created (path + kind: people / projects / institutions / topics / glossary / commitments)")
	fmt.Fprintln(b, "- episodic log written: `brain/episodic/<YYYY-MM-DD>.md` path and bullet count")
	...
	fmt.Fprintln(b, "- superseded fields, with the path, frontmatter key, and `superseded_by` date")
	fmt.Fprintln(b, "- candidates rejected as textbook / public knowledge or as activity-noise, with the term and the rejected note path")
