...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:m="http://schemas.microsoft.com/exchange/services/2006/messages">
  <soap:Body>
	...
	defer client.Close()
	err = client.UpdateInboxRules(t.Context(), []RuleOperation{{Kind: RuleOperationCreate, Rule: Rule{Name: "Move project mail", Priority: 1, Enabled: true, Conditions: RuleConditions{ContainsSubjectStrings: []string{"project"}}, Actions: RuleActions{MoveToFolderID: "inbox", StopProcessingRules: true}}}, {Kind: RuleOperationDelete, Rule: Rule{ID: "rule-1"}}})
	if err != nil {
		t.Fatalf("UpdateInboxRules() error: %v", err)
	}
	for _, snippet := range []string{"<m:UpdateInboxRules>", "<t:CreateRuleOperation>", "<t:ContainsSubjectStrings><t:String>project</t:String></t:ContainsSubjectStrings>", "<t:MoveToFolder><t:DistinguishedFolderId Id=\"inbox\" /></t:MoveToFolder>", "<t:DeleteRuleOperation><t:RuleId>rule-1</t:RuleId></t:DeleteRuleOperation>"} {
