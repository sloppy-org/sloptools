...
var MCPTools = []Tool{
	{Name: "sloppy_mail", Description: "Mail. Actions: message listing/reading, draft (save to Drafts folder), send (send immediately), draft_send (send pre-saved draft by draft_id), reply, flag, filter, OOF, commitments. Use sloppy_tool_help tool=mail for the full action list.", Required: []string{"action"}, Properties: map[string]ToolProperty{
		...
		"folder":             {Type: "string", Description: "Folder/label scope for message_list (e.g. INBOX). Omit to search all folders."},
		"query":              {Type: "string", Description: "Free-text search for message_list. Matches against subject, body, and participants (from/to/cc)."},
		"text":               {Type: "string", Description: "Alias for query."},
		"from":               {Type: "string", Description: "Substring filter on sender name or address for message_list."},
		"participants":       {Type: "string", Description: "Substring filter matching anywhere in From/To/Cc for message_list."},
		"after":              {Type: "string", Description: "Only include messages received at or after this RFC3339 timestamp."},
		"before":             {Type: "string", Description: "Only include messages received strictly before this RFC3339 timestamp."},
		"days":               {Type: "integer", Description: "Lookback window in days for message_list (sets after=now-days)."},
		"has_attachment":     {Type: "boolean", Description: "Only include messages with (true) or without (false) attachments."},
		"is_read":            {Type: "boolean", Description: "Only include read (true) or unread (false) messages."},
		"is_flagged":         {Type: "boolean", Description: "Only include flagged (true) or unflagged (false) messages."},
		"include_spam_trash": {Type: "boolean", Description: "Include junk/trash folders in global searches (default false)."},
		"page_token":         {Type: "string", Description: "Pagination token from next_page_token of a prior message_list call."},
		"limit":              {Type: "integer", Description: "Maximum messages to return for message_list (default 10, capped at 50)."},
	}},
