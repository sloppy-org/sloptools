...
func TestExchangeClientUsesRefreshTokenForGraphOperations(t *testing.T) {
	...
	if err := saveExchangeTokenFile(tokensPath, exchangeToken{RefreshToken: "refresh-token", ExpiresAt: time.Date(2026, time.March, 9, 8, 0, 0, 0, time.UTC)}); err != nil {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/tenant/oauth2/v2.0/token":
			...
			writeJSON(t, w, map[string]any{"access_token": "access-1", "refresh_token": "refresh-2", "token_type": "Bearer", "expires_in": 3600})
		case r.URL.Path == "/v1.0/me/mailFolders":
			requireBearer(t, r, "access-1")
			writeJSON(t, w, map[string]any{"value": []map[string]any{{"id": "inbox-id", "displayName": "Inbox", "wellKnownName": "inbox", "childFolderCount": 0, "totalItemCount": 12, "unreadItemCount": 4}}})
		case r.URL.Path == "/v1.0/me/mailFolders/inbox-id/messages":
			...
			writeJSON(t, w, map[string]any{"value": []map[string]any{{"id": "msg-1", "conversationId": "conv-1", "subject": "Quarterly review", "bodyPreview": "Agenda attached", "isRead": false, "parentFolderId": "inbox-id", "receivedDateTime": "2026-03-09T09:00:00Z", "webLink": "https://example.invalid/mail/msg-1"}}})
		case r.URL.Path == "/v1.0/me/messages/msg-1" && r.Method == http.MethodGet:
			requireBearer(t, r, "access-1")
			writeJSON(t, w, map[string]any{"id": "msg-1", "conversationId": "conv-1", "subject": "Quarterly review", "bodyPreview": "Agenda attached", "isRead": false, "parentFolderId": "inbox-id", "receivedDateTime": "2026-03-09T09:00:00Z"})
	...
	client, err := NewExchangeClient(ExchangeConfig{Label: "Work Mail", ClientID: "client-id", TenantID: "tenant", BaseURL: server.URL, AuthBaseURL: server.URL, TokenPath: tokensPath}, WithExchangeClock(func() time.Time {
		return now
	}), WithExchangeEnvLookup(func(key string) (string, bool) {
	...
	messages, err := client.ListMessages(context.Background(), ListMessageOptions{FolderID: "inbox-id", Top: 5, Select: []string{"id", "subject"}})
	if err != nil {
...
func newExchangeMailProviderForTest(t *testing.T) (*ExchangeMailProvider, *exchangeMailProviderTestActions) {
	...
	provider, err := NewExchangeMailProvider(ExchangeConfig{Label: "Work Mail", ClientID: "client-id", TenantID: "tenant", BaseURL: server.URL, AuthBaseURL: server.URL, TokenPath: tokensPath}, WithExchangeClock(func() time.Time {
...
func exchangeMailProviderFoldersResponse() map[string]any {
	return map[string]any{"value": []map[string]any{{"id": "inbox-id", "displayName": "Inbox", "wellKnownName": "inbox", "totalItemCount": 12, "unreadItemCount": 3, "childFolderCount": 0}, {"id": "contracts-id", "displayName": "Contracts", "wellKnownName": "", "totalItemCount": 4, "unreadItemCount": 1, "childFolderCount": 0}}}
}

func exchangeMailProviderListResponse() map[string]any {
	return map[string]any{"value": []map[string]any{{"id": "msg-1", "conversationId": "conv-1", "subject": "Quarterly review", "bodyPreview": "Please review the budget appendix", "isRead": false, "hasAttachments": true, "flag": map[string]any{"flagStatus": "flagged"}, "parentFolderId": "contracts-id", "receivedDateTime": "2026-03-09T09:00:00Z", "from": map[string]any{"emailAddress": map[string]any{"name": "Ada", "address": "ada@example.com"}}, "toRecipients": []map[string]any{{"emailAddress": map[string]any{"name": "Team", "address": "team@example.com"}}}}, {"id": "msg-2", "conversationId": "conv-2", "subject": "Archive me", "bodyPreview": "No action", "isRead": true, "hasAttachments": false, "flag": map[string]any{"flagStatus": "notFlagged"}, "parentFolderId": "inbox-id", "receivedDateTime": "2026-03-08T09:00:00Z", "from": map[string]any{"emailAddress": map[string]any{"name": "Bob", "address": "bob@example.com"}}}}}
}

func exchangeMailProviderMessageResponse() map[string]any {
	return map[string]any{"id": "msg-1", "conversationId": "conv-1", "subject": "Quarterly review", "bodyPreview": "Please review the budget appendix", "body": map[string]any{"contentType": "html", "content": "<p>Please review the budget appendix by March 12.</p>"}, "isRead": false, "hasAttachments": true, "flag": map[string]any{"flagStatus": "flagged"}, "parentFolderId": "contracts-id", "receivedDateTime": "2026-03-09T09:00:00Z", "webLink": "https://example.invalid/mail/msg-1", "from": map[string]any{"emailAddress": map[string]any{"name": "Ada", "address": "ada@example.com"}}, "toRecipients": []map[string]any{{"emailAddress": map[string]any{"name": "Team", "address": "team@example.com"}}}, "ccRecipients": []map[string]any{{"emailAddress": map[string]any{"name": "Ops", "address": "ops@example.com"}}}}
}
...
func assertExchangeMailProviderSearchAndFetch(t *testing.T, provider *ExchangeMailProvider) {
	...
	ids, err := provider.ListMessages(context.Background(), DefaultSearchOptions().WithFolder("INBOX").WithIsRead(false).WithIsFlagged(true).WithSubject("Quarterly").WithText("budget").WithHasAttachment(true).WithMaxResults(10))
	if err != nil {
		t.Fatalf("ListMessages() error: %v", err)
	}
...
func TestExchangeClientFallsBackToDeviceCodeFlow(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/tenant/oauth2/v2.0/devicecode":
			...
			writeJSON(t, w, map[string]any{"device_code": "device-code", "user_code": "ABCD-EFGH", "verification_uri": "https://microsoft.com/devicelogin", "verification_url": "https://microsoft.com/devicelogin", "message": "Open the browser and enter the code.", "expires_in": 900, "interval": 1})
		case "/tenant/oauth2/v2.0/token":
			...
			if tokenPolls == 1 {
				...
				writeJSON(t, w, map[string]any{"error": map[string]any{"code": "authorization_pending", "message": "authorization pending"}})
				return
			}
			writeJSON(t, w, map[string]any{"access_token": "device-access", "refresh_token": "device-refresh", "token_type": "Bearer", "expires_in": 3600})
	...
	defer server.Close()
	sleepCalls := 0
	client, err := NewExchangeClient(ExchangeConfig{Label: "Work Mail", ClientID: "client-id", TenantID: "tenant", BaseURL: server.URL, AuthBaseURL: server.URL, TokenPath: filepath.Join(t.TempDir(), "exchange.json")}, WithExchangeDeviceCodePrompt(func(info DeviceCodeInfo) error {
		promptInfo = info
		return nil
...
func TestIMAPClient_ListMessages(t *testing.T) {
	...
	defer server.Close()
	server.AddMessage("INBOX", imaptest.TestMessage{Subject: "Test Message 1", From: "alice@example.com", To: "bob@example.com", Body: "Hello, this is a test message."})
	server.AddMessage("INBOX", imaptest.TestMessage{Subject: "Test Message 2", From: "charlie@example.com", To: "bob@example.com", Body: "Another test message."})
	client := NewIMAPClient("test", "127.0.0.1", server.Port(), "test", "password", false, false)
	defer client.Close()
	ctx := context.Background()
