...
func (c *ExchangeClient) ListContacts(ctx context.Context) ([]providerdata.Contact, error) {
	query := url.Values{"$select": {"id,displayName,companyName,emailAddresses,businessPhones"}, "$top": {"200"}}
...
func parseExchangeContact(contact exchangeContact) *providerdata.Contact {
	...
	return &providerdata.Contact{ProviderRef: strings.TrimSpace(contact.ID), Name: name, Email: emailAddress, Organization: strings.TrimSpace(contact.CompanyName), Phones: phones}
}
