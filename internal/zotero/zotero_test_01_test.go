...
func newTestClient(t *testing.T, handler http.HandlerFunc) *Client {
	...
	client, err := NewClient("12345", "api-key-1", WithBaseURL(server.URL), WithHTTPClient(server.Client()))
	if err != nil {
...
func TestListItemsAndCollections(t *testing.T) {
	client := newTestClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case "/users/12345/items":
			...
			_ = json.NewEncoder(w).Encode([]map[string]any{{"key": "ITEM1", "version": 7, "data": map[string]any{"itemType": "journalArticle", "title": "Pragmatic Testing", "DOI": "10.1000/example", "abstractNote": "Short abstract.", "publicationTitle": "Journal of Tests", "creators": []map[string]any{{"creatorType": "author", "firstName": "Ada", "lastName": "Lovelace"}}, "tags": []map[string]any{{"tag": "ml", "type": 0}}}}})
		case "/users/12345/collections":
			if got := r.URL.Query().Get("top"); got != "1" {
				t.Fatalf("top = %q, want 1", got)
			}
			_ = json.NewEncoder(w).Encode([]map[string]any{{"key": "COLL1", "version": 2, "data": map[string]any{"name": "Papers", "parentCollection": ""}}})
		default:
			t.Fatalf("unexpected request %s", r.URL.String())
		}
	})
	items, err := client.ListItems(context.Background(), ListRemoteItemsOptions{CollectionKey: "COLL1", Limit: 5})
	if err != nil {
	...
	collections, err := client.ListCollections(context.Background(), ListRemoteCollectionsOptions{TopLevelOnly: true})
	if err != nil {
...
func buildTestLibrary(t *testing.T, dbPath string) {
	...
	statements := []string{`CREATE TABLE itemTypes (itemTypeID INTEGER PRIMARY KEY, typeName TEXT NOT NULL);`, `CREATE TABLE items (itemID INTEGER PRIMARY KEY, itemTypeID INTEGER NOT NULL, key TEXT NOT NULL, dateAdded TEXT NOT NULL DEFAULT '', dateModified TEXT NOT NULL DEFAULT '');`, `CREATE TABLE deletedItems (itemID INTEGER PRIMARY KEY);`, `CREATE TABLE fields (fieldID INTEGER PRIMARY KEY, fieldName TEXT NOT NULL);`, `CREATE TABLE itemDataValues (valueID INTEGER PRIMARY KEY, value TEXT NOT NULL);`, `CREATE TABLE itemData (itemID INTEGER NOT NULL, fieldID INTEGER NOT NULL, valueID INTEGER NOT NULL);`, `CREATE TABLE creatorTypes (creatorTypeID INTEGER PRIMARY KEY, creatorType TEXT NOT NULL);`, `CREATE TABLE creatorData (creatorDataID INTEGER PRIMARY KEY, firstName TEXT NOT NULL DEFAULT '', lastName TEXT NOT NULL DEFAULT '', name TEXT NOT NULL DEFAULT '');`, `CREATE TABLE creators (creatorID INTEGER PRIMARY KEY, creatorDataID INTEGER NOT NULL);`, `CREATE TABLE itemCreators (itemID INTEGER NOT NULL, creatorID INTEGER NOT NULL, creatorTypeID INTEGER NOT NULL, orderIndex INTEGER NOT NULL);`, `CREATE TABLE collections (collectionID INTEGER PRIMARY KEY, key TEXT NOT NULL, collectionName TEXT NOT NULL, parentCollectionID INTEGER);`, `CREATE TABLE collectionItems (collectionID INTEGER NOT NULL, itemID INTEGER NOT NULL);`, `CREATE TABLE tags (tagID INTEGER PRIMARY KEY, name TEXT NOT NULL);`, `CREATE TABLE itemTags (itemID INTEGER NOT NULL, tagID INTEGER NOT NULL, type INTEGER NOT NULL DEFAULT 0);`, `CREATE TABLE itemAttachments (itemID INTEGER PRIMARY KEY, parentItemID INTEGER, contentType TEXT NOT NULL DEFAULT '', path TEXT NOT NULL DEFAULT '', linkMode INTEGER NOT NULL DEFAULT 0);`, `CREATE TABLE itemAnnotations (itemID INTEGER PRIMARY KEY, parentItemID INTEGER, type TEXT NOT NULL DEFAULT '', authorName TEXT NOT NULL DEFAULT '', text TEXT NOT NULL DEFAULT '', comment TEXT NOT NULL DEFAULT '', color TEXT NOT NULL DEFAULT '', pageLabel TEXT NOT NULL DEFAULT '', sortIndex TEXT NOT NULL DEFAULT '', position TEXT NOT NULL DEFAULT '');`, `INSERT INTO itemTypes (itemTypeID, typeName) VALUES (1, 'journalArticle'), (2, 'attachment'), (3, 'annotation');`, `INSERT INTO items (itemID, itemTypeID, key, dateAdded, dateModified) VALUES (1, 1, 'ITEM-1', '2026-03-08', '2026-03-09'), (2, 2, 'ATT-1', '2026-03-08', '2026-03-09'), (3, 3, 'ANN-1', '2026-03-08', '2026-03-09');`, `INSERT INTO fields (fieldID, fieldName) VALUES (1, 'title'), (2, 'DOI'), (3, 'abstractNote'), (4, 'date'), (5, 'publicationTitle');`, `INSERT INTO itemDataValues (valueID, value) VALUES (1, 'Pragmatic Testing'), (2, '10.1000/example'), (3, 'Short abstract.'), (4, '2026'), (5, 'Paper PDF'), (6, 'Journal of Tests');`, `INSERT INTO itemData (itemID, fieldID, valueID) VALUES (1, 1, 1), (1, 2, 2), (1, 3, 3), (1, 4, 4), (1, 5, 6), (2, 1, 5);`, `INSERT INTO creatorTypes (creatorTypeID, creatorType) VALUES (1, 'author');`, `INSERT INTO creatorData (creatorDataID, firstName, lastName, name) VALUES (1, 'Ada', 'Lovelace', '');`, `INSERT INTO creators (creatorID, creatorDataID) VALUES (1, 1);`, `INSERT INTO itemCreators (itemID, creatorID, creatorTypeID, orderIndex) VALUES (1, 1, 1, 0);`, `INSERT INTO collections (collectionID, key, collectionName, parentCollectionID) VALUES (1, 'COLL-1', 'Papers', NULL);`, `INSERT INTO collectionItems (collectionID, itemID) VALUES (1, 1);`, `INSERT INTO tags (tagID, name) VALUES (1, 'ml');`, `INSERT INTO itemTags (itemID, tagID, type) VALUES (1, 1, 0);`, `INSERT INTO itemAttachments (itemID, parentItemID, contentType, path, linkMode) VALUES (2, 1, 'application/pdf', 'storage:paper.pdf', 1);`, `INSERT INTO itemAnnotations (itemID, parentItemID, type, authorName, text, comment, color, pageLabel, sortIndex, position) VALUES (3, 2, 'highlight', 'Ada', 'Important result', 'Revisit this proof', '#ffd400', '4', '00001|00001|00001', '{\"pageIndex\":3}');`}
