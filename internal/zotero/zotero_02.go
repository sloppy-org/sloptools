...
func (r *Reader) ListItems(ctx context.Context) ([]Item, error) {
	...
	for rows.Next() {
		var item Item
		if err := rows.Scan(&item.ID, &item.Key, &item.ItemType, &item.DateAdded, &item.DateModified, &item.Title, &item.Journal, &item.DOI, &item.ISBN, &item.Abstract, &item.Year); err != nil {
			return nil, err
		}
		out = append(out, item)
...
func (r *Reader) ListAttachments(ctx context.Context, parentKey string) ([]Attachment, error) {
	...
	for rows.Next() {
		var item Attachment
		if err := rows.Scan(&item.ID, &item.Key, &item.ParentKey, &item.Title, &item.ContentType, &item.Path, &item.LinkMode, &item.DateModified); err != nil {
			return nil, err
		}
		out = append(out, item)
	}
...
func (r *Reader) ListAnnotations(ctx context.Context, parentKey string) ([]Annotation, error) {
	...
	for rows.Next() {
		var item Annotation
		if err := rows.Scan(&item.ID, &item.Key, &item.ParentKey, &item.AnnotationType, &item.AuthorName, &item.Text, &item.Comment, &item.Color, &item.PageLabel, &item.SortIndex, &item.Position, &item.DateModified); err != nil {
			return nil, err
		}
		out = append(out, item)
	}
...
func (r *Reader) DetectBetterBibTeXExports() ([]string, error) {
	candidates := []string{filepath.Dir(r.path), filepath.Dir(filepath.Dir(r.path))}
	var matches []string
...
func collectCitationEntries(value any, out *[]citationExportEntry) {
	...
	case map[string]any:
		entry := citationExportEntry{ItemKey: stringValue(firstDefined(typed, "itemKey", "item_key", "key")), CitationKey: stringValue(firstDefined(typed, "citationKey", "citation_key", "citekey")), DOI: stringValue(firstDefined(typed, "DOI", "doi")), Title: stringValue(firstDefined(typed, "title"))}
