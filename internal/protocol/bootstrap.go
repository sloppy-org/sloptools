...
func BootstrapProject(projectDir string) (Result, error) {
	...
	sloptoolsDir := filepath.Join(abs, ".sloptools")
	if err := os.MkdirAll(sloptoolsDir, 0o755); err != nil {
		return Result{}, err
	}
	paths := Paths{
		ProjectDir:    abs,
		MCPConfigPath: filepath.Join(sloptoolsDir, "codex-mcp.toml"),
	}
	_ = os.WriteFile(paths.MCPConfigPath, []byte(fmt.Sprintf("[mcp_servers.sloptools]\ncommand = \"sloptools\"\nargs = [\"mcp-server\", \"--project-dir\", \"%s\"]\n", strings.ReplaceAll(abs, "\\", "\\\\"))), 0o644)
...
func ensureGitignore(projectDir string) error {
	...
	want := ".sloptools/artifacts/\n"
	if strings.Contains(data, ".sloptools/artifacts/") {
		return nil
	}
