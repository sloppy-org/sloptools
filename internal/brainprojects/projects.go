...
func formatOpenLoops(items ProjectBuckets) string {
	...
	writeSection(&b, "Chris owes", items.Next, false)
	writeSection(&b, "Waiting on others", items.Waiting, false)
	writeSection(&b, "Closed (last 14 days)", items.Closed, true)
	return b.String()
}

func writeSection(b *strings.Builder, title string, items []ProjectCommitment, closed bool) {
	if len(items) == 0 {
		return
	}
	b.WriteString("\n### " + title + "\n")
	for _, item := range items {
		...
		writeSuffix(b, item, closed)
		b.WriteByte('\n')
	}
}

func writeSuffix(b *strings.Builder, item ProjectCommitment, closed bool) {
	switch {
