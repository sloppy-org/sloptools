...
func (m *TrainingModel) Score(message Message) localEvidence {
	...
	factors := FactorScores{Spam: m.spam.score(tokens), ActionRequired: m.actionRequired.score(tokens), Skim: m.skim.score(tokens), Reference: m.reference.score(tokens), Staleness: heuristicStaleness(message)}
	signals := []string{fmt.Sprintf("spam=%.2f", factors.Spam), fmt.Sprintf("action=%.2f", factors.ActionRequired), fmt.Sprintf("skim=%.2f", factors.Skim), fmt.Sprintf("reference=%.2f", factors.Reference), fmt.Sprintf("staleness=%.2f", factors.Staleness)}
...
func buildDeterministicRules(reviews []ReviewedExample) []DeterministicRule {
	...
	for _, stat := range collectDominantRules(senderCounts, 4, 1.0) {
		...
		rules = append(rules, DeterministicRule{Scope: "sender", Key: stat.key, Action: toAction(stat.action), Support: stat.total, Purity: float64(stat.dominant) / float64(stat.total), Reason: "exact sender pattern from manual reviews"})
	}
	for _, stat := range collectDominantRules(domainCounts, 8, 1.0) {
		if !domainRuleAllowed(stat.key, stat.action) {
			continue
		}
		rules = append(rules, DeterministicRule{Scope: "domain", Key: stat.key, Action: toAction(stat.action), Support: stat.total, Purity: float64(stat.dominant) / float64(stat.total), Reason: "exact domain pattern from manual reviews"})
	}
...
func buildInconsistentPatterns(reviews []ReviewedExample) []InconsistentPattern {
	...
	for sender, counts := range senderCounts {
		...
		slices.Sort(actions)
		out = append(out, InconsistentPattern{Scope: "sender", Key: sender, Count: total, Actions: actions})
	}
