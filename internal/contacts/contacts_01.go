...
func (g *GmailProvider) ListContacts(ctx context.Context) ([]providerdata. // ListContacts returns every contact in the authenticated account's "My
	...
	pageToken := ""
	for {
		call := svc.People.Connections.List("people/me").Context(ctx).PageSize(1000).PersonFields(personFields)
		if pageToken != "" {
...
func (g *GmailProvider) SearchContacts(ctx context.Context, query string) ([]providerdata. // SearchContacts runs the People API searchContacts endpoint and returns the
...
func (g *GmailProvider) ListContactGroups(ctx context.Context) ([]Group, // ListContactGroups returns the user's contact groups. The "system" groups
	...
	for _, gr := range result.ContactGroups {
		...
		out = append(out, Group{ID: gr.ResourceName, Name: strings.TrimSpace(gr.FormattedName), MemberCount: int(gr.MemberCount)})
	}
	return out, nil
}

func (g *GmailProvider) AddToGroup(ctx context.Context, groupID string, contactIDs []string) error { // AddToGroup appends memberships by patching contactGroups/members:modify.
	...
	_, err = svc.ContactGroups.Members.Modify(strings.TrimSpace(groupID), &people.ModifyContactGroupMembersRequest{ResourceNamesToAdd: contactIDs}).Context(ctx).Do()
	if err != nil {
...
func (g *GmailProvider) RemoveFromGroup(ctx context.Context, groupID string, contactIDs []string) error { // RemoveFromGroup drops memberships from a group.
	...
	g.ensureResourcePrefix(contactIDs)
	_, err = svc.ContactGroups.Members.Modify(strings.TrimSpace(groupID), &people.ModifyContactGroupMembersRequest{ResourceNamesToRemove: contactIDs}).Context(ctx).Do()
	if err != nil {
...
func (g *GmailProvider) GetContactPhoto(ctx context.Context, id string) ([]byte, // GetContactPhoto downloads the primary photo. People puts the photo URL on
...
func contactFromGooglePerson(person *people.Person) *providerdata.Contact {
	...
	contact := &providerdata.Contact{ProviderRef: strings.TrimSpace(person.ResourceName), Name: name, Email: emailAddress, Organization: firstGoogleOrganization(person.Organizations), Phones: allGooglePhones(person.PhoneNumbers), Addresses: allGoogleAddresses(person.Addresses), Notes: firstGoogleBiography(person.Biographies)}
...
func googlePersonFromContact(c providerdata.Contact) *people.Person {
	...
	for _, addr := range c.Addresses {
		pa := &people.Address{Type: addr.Type, StreetAddress: addr.Street, City: addr.City, Region: addr.Region, PostalCode: addr.Postal, Country: addr.Country}
		if pa.Type == "" {
			pa.Type = "other"
		}
...
func allGoogleAddresses(values []*people.Address) []providerdata.PostalAddress {
	...
	for _, value := range values {
		...
		addr := providerdata.PostalAddress{Type: strings.TrimSpace(value.Type), Street: strings.TrimSpace(value.StreetAddress), City: strings.TrimSpace(value.City), Region: strings.TrimSpace(value.Region), Postal: strings.TrimSpace(value.PostalCode), Country: strings.TrimSpace(value.Country)}
...
func firstGoogleBirthday(values []* // firstGoogleBirthday returns the first birthday with a full Y-M-D date.
// Partial birthdays (missing year) are skipped; callers get nil and the
// contact-level field stays unset.
people.Birthday) *time.Time {
	for _, value := range values {
