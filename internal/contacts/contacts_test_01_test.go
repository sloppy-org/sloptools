...
func TestListContactsPaginatesAndMapsFullFields(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			...
			writeJSON(w, map[string]any{"connections": []any{map[string]any{"resourceName": "people/c1", "names": []any{map[string]any{"displayName": "Ada Lovelace"}}, "emailAddresses": []any{map[string]any{"value": "ada@example.com"}}, "organizations": []any{map[string]any{"name": "Analytical Engine"}}, "phoneNumbers": []any{map[string]any{"value": "+11111"}}, "addresses": []any{map[string]any{"streetAddress": "1 Programmer Lane", "city": "London", "postalCode": "W1", "country": "UK", "type": "home"}}, "biographies": []any{map[string]any{"value": "Notes about Ada"}}, "photos": []any{map[string]any{"url": "https://example.invalid/photo.jpg"}}}}, "nextPageToken": "next"})
			return
		}
		if r.URL.Query().Get("pageToken") != "next" {
			t.Fatalf("second call pageToken = %q, want next", r.URL.Query().Get("pageToken"))
		}
		writeJSON(w, map[string]any{"connections": []any{map[string]any{"resourceName": "people/c2", "names": []any{map[string]any{"displayName": "Carol"}}, "emailAddresses": []any{map[string]any{"value": "CAROL@example.com"}}}}})
	}
...
func TestSearchContactsHitsSearchEndpoint(t *testing.T) {
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, map[string]any{"results": []any{map[string]any{"person": map[string]any{"resourceName": "people/c1", "names": []any{map[string]any{"displayName": "Ada"}}, "emailAddresses": []any{map[string]any{"value": "ada@example.com"}}}}}})
	}
	provider := buildStubProvider(t, handler)
...
func TestCreateContactWritesPerson(t *testing.T) {
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, map[string]any{"resourceName": "people/c99", "names": []any{map[string]any{"displayName": "New"}}, "emailAddresses": in.EmailAddresses})
	}
	provider := buildStubProvider(t, handler)
...
func TestUpdateContactFetchesEtagBeforePatching(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && strings.Contains(r.URL.Path, "/v1/people/c1"):
			gets++
			writeJSON(w, map[string]any{"resourceName": "people/c1", "etag": "etag-123", "names": []any{map[string]any{"displayName": "Old"}}, "emailAddresses": []any{map[string]any{"value": "old@example.com"}}})
		case r.Method == http.MethodPatch && strings.Contains(r.URL.Path, "/v1/people/c1:updateContact"):
			...
			writeJSON(w, map[string]any{"resourceName": "people/c1", "names": []any{map[string]any{"displayName": "Updated"}}, "emailAddresses": []any{map[string]any{"value": "updated@example.com"}}})
		default:
			t.Fatalf("unexpected %s %s", r.Method, r.URL.Path)
		}
	}
...
func TestListContactGroupsMapsFormattedName(t *testing.T) {
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, map[string]any{"contactGroups": []any{map[string]any{"resourceName": "contactGroups/myContacts", "formattedName": "My Contacts", "memberCount": 10}}})
	}
	provider := buildStubProvider(t, handler)
