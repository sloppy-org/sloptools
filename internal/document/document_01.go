...
type latexBuilder struct{ cfg BuildConfig }

type pandocBuilder struct{ cfg BuildConfig }
...
func BuildWorkspaceDocument(ctx context.Context, sourceDir, requestedPath string) (BuildResult, error) {
	...
	return BuildResult{PDFPath: artifactPath, MainFile: filepath.ToSlash(mainFile), Builder: builderName}, nil
}
...
func resolveBuilder(sourceDir, requestedPath string, cfg BuildConfig) (string, string, Builder, error) {
	builders := map[string]Builder{"latex": latexBuilder{cfg: cfg}, "pandoc": pandocBuilder{cfg: cfg}}
	if requestedPath != "" {
